...
var control = false;
var headerMsg =  `Boşluk tuşuna bir kere bas, merdiven yeterince uzayınca tekrar basarak durdur!`;
...
function draw() {
  if (selectPlayerMenuSkipped) {
    if (control) {
      ...
      if(isMobileDevice()) {
          headerMsg =  `Ekranda bir yere bas, merdiven yeterince uzayınca tekrar bir yere basarak durdur!`
        ...
        if (
          ...
          if (soldierPos.x + soldier.width >= stickEndPos) {
            ...
            var confirmBtn = confirm(
              `Üzgünüm asker, bu surlara daha fazla dayanamadık.`
            );
